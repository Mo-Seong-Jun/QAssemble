...
class MPIManager(object):
    ...
    def Query(self, nodeinfo : dict):

        nprock = nodeinfo.get('nprock', 1)
        nprocf = nodeinfo.get('nprocf', 1)
        
        nk = nodeinfo['nk']
        fnode = nodeinfo['fnode']
        bnode = nodeinfo['bnode']
        shape = nodeinfo['shape']

        if (nk, fnode, bnode, nprock, nprocf) in self.mpidict:
            return self.mpidict[(nk, fnode, bnode, nprock, nprocf)]
        else:
            nodedict = {}
            ...
            btemp = np.arange(bnode)
            bchunk = np.array_split(btemp, nprocf)
            submatrixb = [(chunk[0], chunk[-1]+1) for chunk in bchunk]
            nodedict['submatrixb'] = submatrixb
            bloc = self.mf.FTLocalGlobal(submatrixb)
            self.mf.bloc = bloc
